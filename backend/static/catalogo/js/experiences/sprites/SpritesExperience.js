/**
 * @file SpritesExperience.js
 * @description Controlador de la experiencia "Sprites Instanciados 3D" (Three.js WebGL/WebGPU inspired).
 */

import * as THREE from 'https://cdnjs.cloudflare.com/ajax/libs/three.js/r128/three.module.js';
import { SpriteCloud } from './SpriteCloud.js';

export class SpritesExperience {
    /**
    ...
    _initThreeScene() {
        ...
        // Crear la nube de 5,000 sprites instanciados
        this.spriteCloud = new SpriteCloud(this.scene, 5000);
    }

    /**
     * Sintetizador Web Audio API para notas armónicas de cristal.
     * @private
     */
    _initAudio() {
        try {
    ...
    _playCrystalChime(normY) {
        ...
        try {
            ...
            const baseFreq = 220 + (1 - normY) * 660; // Frecuencias cristalinas
    ...
    /**
     * Mapea coordenadas 2D normalizadas de pantalla a coordenadas 3D en el plano del mundo.
     * @private
     */
    _screenToWorld3D(normX, normY) {
        ...
        // Proyección simple basada en la distancia de la cámara
    ...
    _onInputEvent(event) {
        ...
        if (event.type === 'pointerdown' || event.type === 'external') {
            ...
            if (this.spriteCloud) {
                this.spriteCloud.triggerImpulse(world3D.x, world3D.y, world3D.z);
            }
            this._playCrystalChime(event.normY);
    ...
    _loop(timestamp) {
        ...
        if (this.spriteCloud) {
            this.spriteCloud.update(timestamp, active3DList);
        }
    ...
    triggerWave(x, y) {
        if (this.inputManager) {
            this.inputManager.triggerExternal(x, y, 'lidar_sprites_api');
        }
    }
